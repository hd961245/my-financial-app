    ...
    try {
        ...
        if (isChinese) {
            if (symbolParam.includes("台積電")) querySymbol = "2330.TW";
                ...
                return NextResponse.json({ error: `請使用標準股票代號進行分析 (無法解析: ${symbolParam})` }, { status: 400 });
            }
        }
        ...
        try {
            ...
            return NextResponse.json({ error: `找不到代號 ${querySymbol} 的即時資訊` }, { status: 404 });
        }
        ...
        if (!historical || historical.length === 0) {
            return NextResponse.json({ error: "無法取得歷史價格進行技術分析" }, { status: 400 });
        }
