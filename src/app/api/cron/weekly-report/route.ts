...
export async function GET(request: Request) {
    ...
    try {
        ...
        try {
            ...
            await fetch(`${protocol}://${host}/api/analyze/sync-watchlist`, { method: 'POST' });
        } catch (syncErr) {
            console.warn('Watchlist Auto-Sync inside Weekly Cron failed, continuing...', syncErr);
        }
...
- **近期新聞焦點**：${analysis.news.length > 0 ? analysis.news[0].title : '無'}`.trim());
        }
...
3. 接著，將這些股票分類為：
   ...
   - 🔴 **弱勢警戒區** (空頭排列)
   並在分類下條列說明原因。
8. 如果有設定目標價且已經非常接近（差不到 5%），請在最後加上「🎯 達標預警」重點提醒。
9. 【重要】請務必使用「繁體中文 (Traditional Chinese)」撰寫整份報告。
10. 使用 Markdown 格式。`;

        const userPrompt = `這是本週自選股的量化掃描結果：\n\n${analysisReports.join('\n\n')}\n\n請根據這些數據生成健檢報告！`;
